...
export function exportGraphAsPNG(
  ...
  if (!extract) {
    alert('PixiJS extract is not available on the renderer. Please ensure you are using pixiApp.renderer and PixiJS v8+.');
...
export function exportGraphAsSVG(graphContainer: any, filename = 'graph.svg') {
  // Assume graphContainer is a NetworkGraph with getNodes() and getEdges()
  const nodes = typeof graphContainer.getNodes === 'function' ? graphContainer.getNodes() : [];
  const edges = typeof graphContainer.getEdges === 'function' ? graphContainer.getEdges() : [];
  if (!nodes.length) {
  ...
  svg += '<g stroke="#888" stroke-width="1">\n';
  // Draw edges
  ...
  // Draw nodes
  svg += '<g stroke="#222" stroke-width="2">\n';
  for (const node of nodes) {
    ...
    if (node.label) {
      svg += `<text x="${node.position.x}" y="${node.position.y + 4}" font-family="Arial, sans-serif" font-size="${Math.max(10, Math.round(node.radius * 1.2))}" fill="#fff" text-anchor="middle" alignment-baseline="middle">${escapeXml(node.label)}</text>\n`;
    }
  }
  svg += '</g>\n';
